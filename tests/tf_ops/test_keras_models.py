from src.tf_ops.models.keras_models import SimpleDenseClassifier, SimpleResNetClassifier
...
class TestSimpleResNetClassifier(tf.test.TestCase):
    def setUp(self):
        self.model_params = {
            "units_1": 8,
            "drop_rate": 0.2,
    ...
    def test_simple_resnet_classifier(self):

        model = SimpleResNetClassifier(self.model_params)
        model.compile(optimizer='adam',
                    loss='binary_crossentropy',
                    metrics=['accuracy'])
